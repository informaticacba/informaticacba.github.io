							...
							if (validator.settings.onsubmit) {
								...
								this.submit( function(event) {
									if (validator.settings.debug)
										// prevent form submit to be able to see
										// console output
									...
									// prevent submit for invalid forms or
			...
			.extend(
					$.validator,
					{
						...
						messages : {
							...
							tandem :"No puede incluir caracteres especiales.",
							remote :"Corregir este campo.",
							email :"Ingresar una direcci&oacute;n de correo v&aacute;lida.",
							url :"Ingresar una URL v&aacute;lida.",
							date :"Ingresar una fecha v&aacute;lida.",
							dateISO :"Please enter a valid date (ISO).",
							dateDE :"Bitte geben Sie ein gültiges Datum ein.",
							number :"Ingresar un n&uacute;mero v&aacute;lido.",
							numberDE :"Bitte geben Sie eine Nummer ein.",
							digits :"Ingresar solo d&iacute;gitos",
							creditcard :"Ingresar una tarjeta de cr&eacute;dito v&aacute;lida.",
							equalTo :"Ingresar el mismo valor nuevamente.",
							accept :"Ingresar un valor con la misma estensi&oacute;n.",
							maxlength :$
									.format("No ingresar m&aacute;s de {0} caracteres."),
							minlength :$
									.format("Ingresar al menos {0} caracteres."),
							rangelength :$
									.format("Ingresar un valor de longitud mayor a {0} y menor a {1}."),
							range :$
									.format("Ingresar un n&uacute;mero entre {0} y {1}."),
							max :$
									.format("Ingresar un valor menor o igual a {0}."),
							min :$
									.format("Ingresar un valor mayor o igual a {0}.")
						},
						...
						methods : {
							...
							email : function(value, element) {
								...
								return this.optional(element)
										|| new RegExp("^([a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\\.)+[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)$")
												.test(value);
							},

							especial :function(value, element){
								var a = new RegExp("^[a-zA-Z0-9@_\.-]+$").test(value);
								return this.optional(element) || a;
							},
							tandem: function(value, element) {
								// validacion de caracteres especiales para tandem
								var a = new RegExp("[^a-zA-Z0-9\\s]").test(value);
								return !a;
							},
...
function validaCuit(cuit) {
	...
	// No estamos validando si el campo esta vacio, eso queda para el "required"
	if (cuit == '') {
		return true; 
	}
	...
	if (cuit.length != 11) {
		...
		for ( var i = 0; i < mult.length; i++) {
			...
			//descrip += "total += ("+cuit.charAt(i)+" = "+parseInt(cuit.charAt(i))+") * "+ mult[i]+"\n";
		}
		var mod = total % 11;
...
function validaCBU(par_cbu)
{
	if(par_cbu.length < 22)
	{
		return false
	}

	var num_cbu =new Array
	bloque1=new Array
	bloque2=new Array;
	ponderador1=new Array
	ponderador2=new Array
	aux=new Array
	digito=""

	for(i=0;i<=7;i++)
	{
		bloque1[i]=par_cbu.charAt(i)
	}	

	for(i=8;i<=21;i++)
		bloque2[i-8]=par_cbu.charAt(i)
	...
	for(k=0;k<=6;k++)
	{
		aux_total=bloque1[k]*ponderador1[k]
		total=parseInt(total)+aux_total
	}

	stotal=""+total
	u=stotal.charAt(stotal.length -1)
	...
	for(k=0;k<=12;k++)
	{
		aux_total=bloque2[k]*ponderador2[k];
		total=parseInt(total)+aux_total
	}
...
function validaDNI(dni) {
	if (typeof(dni) == 'undefined') return false;
	// Vacio es valido, requerido se valida con required.
	if (dni.length == 0) return true; 
	if (dni.length < 6 || dni.length > 8) return false;
	if (parseInt(dni, 10) <= 0) return false;
	var validaRegExpression = /^(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(dni);
    return validaRegExpression;
}

function validaCuitDni(identificacion) {
    return validaCuit(identificacion) || validaDNI(identificacion);
}
...
function validaNumeroDecimal(numero) {
	return /^[0-9]*$/.test(numero) || /^[0-9]+\.[0-9]$/.test(numero) || /^[0-9]+\.[0-9][0-9]$/.test(numero);
}

function validarExpresionRegular(valor, elements, params) {
	return new RegExp(params).test(valor);	
}

function validafechaSuperiorAOtraFecha(valor, elements, params) {
	var fecha1 = valor.split("/");
	var fecha1s = fecha1[2] + fecha1[1] + fecha1[0];
	var fecha2 = $(params).val().split("/");
	var fecha2s = fecha2[2] + fecha2[1] + fecha2[0];
	return fecha2s <= fecha1s;
}

function validarNoSoloNumerico(valor, elements, params) {
	return !(new RegExp('^[0-9]+$').test(valor));	
}

function validarRequiredSiCondicionTrue(valor, elements, params) {
	return !((valor == null || valor == '') &&  ($("#"+params).val() == 'true'));	
}

// Params es una funcion
function validarRequeridoSi(valor, elements, params) {
	return !( (valor == null || valor == '') && (params) );	
}

function validaAliasCbu(alias){
	return ((alias != null && alias != "") && (alias.match(/^[a-z0-9\.\-]+$/i)) && (alias.length >= 6 && alias.length <= 20));
}

function validarNumerico(valor, elements, params) {
	return new RegExp(params).test(valor);	
}


$.validator.addMethod("cuit", validaCuit, 'CUIT/CUIL/CDI Inv&aacute;lido');
$.validator.addMethod("cuil", validaCuit, 'CUIT/CUIL/CDI Inv&aacute;lido');
$.validator.addMethod("cbu", validaCBU, 'CBU Inv&aacute;lido');
$.validator.addMethod("dni", validaDNI, 'DNI Inv&aacute;lido');
$.validator.addMethod("cuitdni", validaCuitDni, 'CUIT/CUIL/CDI/DNI Inv&aacute;lido');
$.validator.addMethod("clave", validaClave, 'Clave Inv&aacute;lida');
$.validator.addMethod("decimales", validaNumeroDecimal, 'Debe tener no m&aacute;s de 2 decimales');
$.validator.addMethod("regexp", validarExpresionRegular, 'El valor ingresado no es v&aacute;lido');
$.validator.addMethod("fechaSuperiorA", validafechaSuperiorAOtraFecha, 'La fecha hasta debe ser posterior a la fecha desde');
$.validator.addMethod("validarRequiredSiCondicionTrue", validarRequiredSiCondicionTrue, 'Campo requerido');
$.validator.addMethod("noSoloNumerico", validarNoSoloNumerico, 'No puede ser solamente num&eacute;rico');
$.validator.addMethod("requiredIf", validarRequeridoSi, 'Campo requerido');
$.validator.addMethod("numerico", validarNumerico, 'El valor debe ser num&eacute;rico');
